...
export default function ItemList({items,onItemSelect})
{
  ...
  const sortedItems = [...items].sort((a,b)=> a[sortBy].localeCompare(b[sortBy]));
      return(
        <div> 
        <div>
          <button onClick={()=>setSortBy('name') }  className={`py-2 px-2 mt-2 ml-4 ${sortBy == 'name' ? 'bg-orange-700 hover:bg-gray-400' : 'bg-orange-300'} hover:bg-gray-700 rounded-md text-black w-full max-w-xs`}>Name</button>
          <button onClick={()=>setSortBy('category')} className={`py-2 px-2 ml-4 mt-2 ${sortBy == 'category' ? 'bg-orange-7 00 hover:bg-gray-400 ': 'bg-orange-300'} hover:bg-gray-700 rounded-md text-black w-full max-w-xs`}>Category</button>
